...
- This version enables the inclusion of an extra pose topic during the measurement update stage
- The parameter 'method_type' in the file 'ekf_localizer.launch' allows the selection of:
  - one single localization source:
    - '0' to update the pose using the topic 'in_pose1'
    - '1' to update the pose using the topic 'in_pose2'
  - fuse the data from two different localization sources:
    - '2' to update the pose sequentially using the topics 'in_pose1' and 'in_pose2'
- The parameter 'use_synchronization' in the file 'ekf_localizer.launch' allows the time synchronization
  - This helps to match the timestamp of the topics 'in_pose1' and 'in_pose2'
  - This would help measurements to be aligned in time for an accurate data fusion process
- This version allows the definition of weight values (which must vary from 0 to 1)
  - This uses the topics 'in_weight1' and 'in_weight2' as inputs
  - This modifies the values 'pose1_gate_dist' and 'pose2_gate_dist'

## Run the package

1. To test this version, utilize the materials provided in this repository in place of the 'ekf_localizer' package provided by Autoware
2. To run this package, launch Autoware using the standard procedure
3. **NOTE:** Run this package together with the 'sensor_weight.launch' provided in [ekf-localization](https://github.com/ACCESSLab/ekf-localization)

## Performance

The weight magnitudes should be related to the uncertainty of each sensor, and it is recomended to use it when pose topics lack of covariance information. In the case of pose topics contain covariance information, these magnitudes should remain at '1' (i.e., the topics 'in_weight1' and 'in_weight2' do not be considered)

Next, it can be seen the performance of the proposed approach compared with the original approach provided by Autoware while driving in LiDAR-limited environments (e.g., the vehicle is driving in a zone where the PCD map is outdated (like this example), or the LiDAR device is failing, or the points raw data from the LiDAR is being corrupted, etc.)

| Original approach | Proposed approach |
| ---      | ---      |
|![Alt Text](media/ekf0_lidar_limited.gif)| ![Alt Text](media/ekf1_lidar_limited.gif) |

A more detailed assessment of the proposed approach using real data can be found in the folder [notebooks](notebooks) of this repository in the file [figures_itsc_2023](notebooks/figures_itsc_2023.ipynb)

## Acknowledgments
  Thanks for [ekf_localizer](https://github.com/astuff/autoware.ai-core_perception.git)
